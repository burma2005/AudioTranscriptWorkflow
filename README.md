...
使用本地端 **openai-whisper** 將錄音語音自動轉錄為 Markdown 逐字稿，再由 AI Agent 智能摘要成結構化會議紀錄。**轉錄階段完全本機執行**，音訊不上傳任何雲端服務；會議紀錄摘要階段則由 AI Agent（雲端）負責整理。

## ✨ 特色

- **轉錄本地化**：Whisper 模型在本機執行，音訊不上傳任何雲端服務
